...
function playCombatAudio(profile, hit, broadcast) {
  ...
  if (profile.sound && typeof helper?.play === "function") {
    const primary = helper.play({ src: profile.sound, volume: 0.48, autoplay: true, loop: false }, broadcast);
    const outcome = `${SYSTEM_AUDIO}/melee-${hit ? "hit" : "miss"}.wav`;
    const timer = globalThis.setTimeout;
    if (typeof timer === "function" && !profile.sound.endsWith(`melee-${hit ? "hit" : "miss"}.wav`)) {
      timer(() => void helper.play({ src: outcome, volume: hit ? 0.32 : 0.2, autoplay: true, loop: false }, broadcast), Math.max(60, Math.round(profile.duration * 0.7)));
    }
    return primary;
  }
...
export function movementEffectProfile(mode = "walk") {
  const normalized = ["run", "jump"].includes(mode) ? mode : "walk";
  return {
    mode: normalized,
    steps: normalized === "run" ? 4 : normalized === "jump" ? 1 : 2,
    interval: normalized === "run" ? 150 : normalized === "jump" ? 420 : 280,
    frequency: normalized === "run" ? 72 : normalized === "jump" ? 115 : 58,
    duration: normalized === "jump" ? 620 : normalized === "run" ? 560 : 620
  };
}

export function playMovementEffect(mode = "walk", { audio = true } = {}) {
  if (!audio) return false;
  const profile = movementEffectProfile(mode);
  const AudioContextClass = globalThis.AudioContext ?? globalThis.webkitAudioContext;
  if (!AudioContextClass) return false;
  const context = new AudioContextClass();
  for (let step = 0; step < profile.steps; step += 1) {
    const start = context.currentTime + (step * profile.interval / 1000);
    const oscillator = context.createOscillator();
    const gain = context.createGain();
    oscillator.type = profile.mode === "jump" ? "sawtooth" : "square";
    oscillator.frequency.setValueAtTime(profile.frequency + (step % 2 ? 8 : 0), start);
    oscillator.frequency.exponentialRampToValueAtTime(Math.max(28, profile.frequency * 0.45), start + 0.12);
    gain.gain.setValueAtTime(profile.mode === "run" ? 0.055 : 0.075, start);
    gain.gain.exponentialRampToValueAtTime(0.001, start + 0.14);
    oscillator.connect(gain).connect(context.destination);
    oscillator.start(start);
    oscillator.stop(start + 0.15);
  }
  globalThis.setTimeout?.(() => void context.close(), profile.duration + 120);
  return true;
}
